<script lang="ts">
    ...
    let {
        ...
        rowColors = ["bg-surface-50-950", "bg-surface-100-900"]
    }: Props = $props();
</script>
...
{#if delegates.pending}
    ...
    {:else}
        <div class={["overflow-y-hidden h-full grid w-full border", borderColor]}>
            <!-- eslint-disable-next-line svelte/require-each-key -->
            {#each Array.from({ length: 20 }) as _, i}
                <div class={["grid grid-cols-[2fr_1fr] h-18", rowColors[+!(i % 2)]]}>
...
{:else if delegates.length}
    <VirtualList items={delegates} viewportClasses={["grid w-full bg-surface-200-800 border", borderColor]} viewportBuffer={200}>
        {#snippet rows(visible)}
            {#each visible as { data: attrs, index: i } (attrs.id)}
                <div class={["grid grid-cols-2", rowColors[+!(i % 2)]]}>
